...
blm_tweets_5000 <- blm_tweets[1:5000,]
...
blm_tweets_sum <- blm_tweets_5000 %>% group_by(date) %>% summarise(text = paste(text, collapse = " "))
...
#I think I was trying to get the code to run when you showed this, so I'm a little confused on how to interpret the graphs, but the best number of topics looks lower now.

#### Do you think that the number of topics is linear in the number of documents??
#I think it is probably not a linear relationship because more documents are likely to repeat some of the same topics.
