...
md_dfm <- tokens(data_char_mobydick,remove_punct = TRUE)
md_dfm2 <- dfm(md_dfm,remove=stopwords("english"))
md_Features <- topfeatures(md_dfm2, 100)
topMd <- data.frame(
  list(
    term = names(md_Features),
    frequency = unname(md_Features)
  )
)
topMd$term <- with(topMd, reorder(term, -frequency))
topMd$rank <- seq(1:100)
ggplot(topMd) + geom_smooth(aes(x=rank, y=frequency), color="blue") +
  theme(axis.text.x=element_text(angle=90, hjust=1))
