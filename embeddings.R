...
setwd("~/GitHub/TAD_2021-master/R lessons")
...
author<-  word_vectors["music", , drop = FALSE] - 
  word_vectors["pop", , drop = FALSE] +
  word_vectors["rock", , drop = FALSE]
cos_sim <- textstat_simil(as.dfm(word_vectors), y = as.dfm(author),
                          margin = "documents", method = "cosine")
head(sort(cos_sim[, 1], decreasing = TRUE), 5)

author<-  word_vectors["wine", , drop = FALSE] - 
  word_vectors["beer", , drop = FALSE] +
  word_vectors["merlot", , drop = FALSE]
cos_sim <- textstat_simil(as.dfm(word_vectors), y = as.dfm(author),
                          margin = "documents", method = "cosine")
head(sort(cos_sim[, 1], decreasing = TRUE), 5)
